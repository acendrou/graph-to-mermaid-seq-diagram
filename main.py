...
class MermaidSeqDiagram:
    ...
    def add_sequence(self, actor1: str, actor2: str, type_arrow: str, text: str) -> None:
        content_temp = self.add_tab() + str(actor1) + type_arrow + str(actor2) + self.add_colon() + text + self.add_newline()
...
def generate_mermaid_seq() -> str:
    c = MermaidSeqDiagram()
    c.add_sequence(actor1="Alice", actor2="John", type_arrow='->>', text='Hello John, how are you?')
    c.add_sequence(actor1="John", actor2="Alice", type_arrow='-->>', text="Great!")
    return c


def write_file(content: str):
    with open('mermaid-seq-diagram.txt', 'w') as f:
        f.write(str(content))
...
def generate_mermaid_seq_diagram_from_graph(G: nx.Graph) -> str:
    ...
    for e in G.edges(data=True):
        print(e)
        c.add_sequence(actor1=e[0], actor2=e[1], type_arrow=e[2]['type_arrow'], text=e[2]['text'])
    return str(c)
